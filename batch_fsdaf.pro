...
pro  batch_fsdaf,File_Name1,File_Name2,File_Name5,year,fsdaf_weight_name_r,d_p,index_i
  ...
  min_class=2.0                ;set the estimated minimum and maximum number of classes
  max_class=3.0
  num_pure=100                 ;number of most purest coarse pixels in each class selected fro change calculation
  DN_min=-1.0                 ;set the range of DN value of the image,If byte, 0 and 255
  DN_max=1.0
  ...
  background=-2                 ;set the value of background pixels. 0 means that pixels will be considered as background if one of its bands= 0
  ...
  print,'open the fine image of the first pair'
  ...
  print,'open the coarse image of the first pair'
  ...
  ;-----------------------------------------------------------
  FileName5 = File_Name5
  print,'open the coarse image of the prediction time'
  ...
  for isub=0,n_ns*n_nl-1,1 do begin
    ...
    if (num_class gt 0) then begin   ;do not process if the whole subset is background
      ...
      k_l[*,*]=abs(temp_kl)
      envi_write_envi_file,k_l, out_name=fsdaf_weight_name_r, data_type=4,$
  ...
  out_name='G:\final_data\evergreen_ndvi\FSDAF_other_other\'+'mod09a1_'+strtrim(year,1)+'_'+strtrim(d_p,1)+'_FSDAF_'+strtrim(index_i,1)
  envi_doit, 'mosaic_doit', fid=mfid, pos=mpos, $
    dims=mdims, out_name=out_name, xsize=xsize, $
    ysize=ysize, x0=x0, y0=y0, georef=0,MAP_INFO=map_info, $
